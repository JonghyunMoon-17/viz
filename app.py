...
st.divider()
st.subheader("추가 검증: 가격–수요 선행-후행 분석")

st.markdown("""
**분석 목적**  
가격 상승이 수요(거래)를 선행하는지, 혹은 수요 변화가 가격을 먼저 자극하는지를 분기별 변화율 기반으로 검증.
""")
...
st.markdown("**선행-후행 상관 분석 결과**")
st.dataframe(lead_lag_result, use_container_width=True)

st.markdown("""
- lag=3개월 분석 결과 오히려 단기적으로는 거래 변화가 가격 변화보다 **조금 더 앞서 나타나는 경향**이 관찰됨
""")
...
st.caption(
    "※ 본 분석을 메인 결과의 강건성을 점검하기 위한 보조 실험(Robustness Check)으로 제시"
)
