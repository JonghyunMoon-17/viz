...
**사분면 해석(핵심):**
- **Q1 (X+, Y+)**: 가격↑에도 거래 비중↑ → *되먹임 구조(가설 지지)*
- **Q4 (X+, Y-)**: 가격↑인데 비중↓ → *기저효과/희석 가능성(예: 강남구)*
- **Q2 (X-, Y+)**: 가격 약세/정체인데 비중↑ → *대체 수요 가능성*
- **Q3 (X-, Y-)**: 약세 지역
        """
    )

    st.info(
        "※ 강남구의 비중 감소는 ‘수요 이탈’이라기보다, 전체 거래량 확대 속 상대적 비중이 희석된 결과일 수 있다."
        "핵심은 Q1에 위치한 ‘가격↑+비중↑’ 지역이 존재하는지와 그 분포로, Q1에 ‘동작/성동/마포/광진’ 같은 준고가 축이 존재한다는 점, 이는 가격 신호가 수요를 억제하지 못하는 ‘되먹임 구조’를 시사합니다."
    )
...
st.caption("Tip: 버블 크기=2025 거래건수로 ‘시장 규모가 큰 구’의 움직임까지 함께 해석할 수 있습니다.")